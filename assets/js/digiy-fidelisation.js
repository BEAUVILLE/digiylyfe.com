...
(function(){
  ...
  function init(){
    ...
    var anchor=document.getElementById('carte-digiy');
    if(!anchor)return;
    var style=document.createElement('style');
    style.id='digiy-fidelisation-style';
    style.textContent='.fidelisationBox{margin-top:12px;padding:16px;border-radius:24px;border:2px solid rgba(45,212,191,.58);background:radial-gradient(420px 190px at 100% 0,rgba(246,196,83,.20),transparent 64%),linear-gradient(145deg,rgba(9,72,49,.96),rgba(4,25,18,.98));box-shadow:0 16px 42px rgba(0,0,0,.28)}.fidelisationLabel{display:inline-flex;padding:6px 10px;border-radius:999px;border:1px solid rgba(246,196,83,.48);background:rgba(246,196,83,.10);color:#fff3cf;font-size:10px;font-weight:1000;letter-spacing:.08em}.fidelisationBox h2{margin:10px 0 7px;color:#fffaf0;font-size:clamp(22px,4.8vw,34px);line-height:1;font-weight:1000;letter-spacing:-.035em}.fidelisationBox p{margin:0;color:rgba(255,250,240,.80);font-size:12.5px;line-height:1.45;font-weight:880}.fidelisationQuote{margin-top:11px!important;padding:10px 12px;border-radius:16px;border:1px dashed rgba(246,196,83,.48);background:rgba(246,196,83,.08);color:#fff3cf!important;text-align:center;font-weight:1000!important}.fidelisationItems{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;margin-top:10px}.fidelisationItems span{min-height:58px;padding:9px;border-radius:15px;border:1px solid rgba(255,255,255,.13);background:rgba(255,255,255,.055);display:flex;align-items:center;gap:7px;color:#eafff3;font-size:10.5px;line-height:1.25;font-weight:950}.fidelisationItems b{color:#bdf7d2;font-size:16px}@media(max-width:760px){.fidelisationItems{grid-template-columns:repeat(2,minmax(0,1fr))}.fidelisationItems span:last-child{grid-column:1/-1}}@media(max-width:420px){.fidelisationBox{padding:13px 11px;border-radius:21px}.fidelisationItems{grid-template-columns:1fr}.fidelisationItems span:last-child{grid-column:auto}}';
