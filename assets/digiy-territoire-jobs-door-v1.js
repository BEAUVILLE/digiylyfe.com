/* DIGIYLYFE — TERRITOIRE · porte EMPLOI / DIGIY JOBS V1
 * Tous territoires : transmet territory + local + lang au MASTER DIGIY JOBS.
 */
(function(){'use strict';
...
function target(){var p=params(),u=new URL('https://jobs.digiylyfe.com/master.html'),territory=p.get('zone')||'',local=p.get('local')||'';if(territory)u.searchParams.set('territory',territory);if(local)u.searchParams.set('local',local);u.searchParams.set('lang',lang());return u.toString()}
function install(){var root=document.getElementById('needs');if(!root)return;var buttons=Array.prototype.slice.call(root.querySelectorAll('button.need'));var hit=buttons.find(function(b){var ic=b.querySelector('strong');return ic&&ic.textContent.trim()==='💼'});if(!hit||hit.hasAttribute('data-digiy-jobs-door'))return;hit.setAttribute('data-digiy-jobs-door','1');hit.setAttribute('aria-label','EMPLOI & MISSIONS · DIGIY JOBS');hit.title='DIGIY JOBS';hit.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();location.href=target()},true)}
