...
function target(){var u=new URL('https://mane-gning.digiylyfe.com/');u.searchParams.set('lang',lang());return u.toString()}
function install(){
  if(!active())return;
  var root=document.getElementById('needs');if(!root)return;
  var old=root.querySelector('[data-digiy-cleaning-door]');if(old)old.remove();
  var x=TEXT[lang()]||TEXT.fr;
  var a=document.createElement('a');a.className='need';a.href=target();a.setAttribute('data-digiy-cleaning-door','1');a.setAttribute('aria-label',x.title+' · MANÉ & GNING');a.style.textDecoration='none';a.style.borderColor='rgba(94,234,212,.62)';a.style.background='linear-gradient(145deg,rgba(94,234,212,.13),rgba(34,197,94,.10))';
  var ic=document.createElement('strong');ic.textContent='🧹';
  var title=document.createElement('span');title.textContent=x.title;
  var meta=document.createElement('small');meta.textContent=x.meta;meta.style.cssText='display:block;margin-top:6px;color:rgba(255,250,240,.78);font-size:10px;line-height:1.3;font-weight:850';
